...
export type ActionHandler<T,U=any> = (ev: ActionEvent) =>
  | undefined
  | ((data: ClonedEvent<U>) => undefined | PartialState<T>)

export type Action<T,U=any> = {
  [P in (keyof HTMLElementEventMap | 'custom')]?: ActionHandler<T,U>
}
...
type ActionEvent = UIEvent & InputEvent & MouseEvent & KeyboardEvent & ClipboardEvent & {
  currentTarget:  HTMLElement & HTMLInputElement & HTMLMediaElement & {
    files: FileList;
  }
}
