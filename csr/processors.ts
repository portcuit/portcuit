...
export const actionProc = <T>(source$: Observable<ActionDetail>, sink: Sink<DeepPartial<T>>) =>
  source$.pipe(
    map(([fn, data]) =>
      sink(new Function(`return ({ReplaceObject, ReplaceArray, EphemeralBoolean, splice, padArray}) => ${fn};`)()
      ({ReplaceObject, ReplaceArray, EphemeralBoolean, splice, padArray})(data))))
