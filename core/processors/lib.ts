...
export const latestMergeMapProc = <T, U, V extends readonly any[], W extends U> (
  ...
  source$.pipe(
    withLatestFrom(...latests$),
    mergeMap((data) => {
      const stream$ = from(fn(data as [T, ...V])).pipe(
        map((data) => sink(data)))
...
export const fromEventProc = <T, U> (source$: Observable<T>, sink: Sink<U>, eventName: string, resultSelector = (...args: any[]) => (args.length === 1 ? args[0] : args) as any) =>
  source$.pipe(
